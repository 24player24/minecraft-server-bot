...
function startStatusLogger() {
  ...
  statusTimer = setInterval(() => {
    if (bot && bot.entity) {
      log(`💓 HP:${bot.health != null ? bot.health.toFixed(1) : '?'}/20 | Food:${bot.food ?? '?'} | Fails:${failStreak}`);
    }
  }, STATUS_MS);
}
...
function handleCommand(username, raw) {
  ...
  if (msg === '!status') {
    bot.chat(`HP:${bot.health != null ? bot.health.toFixed(1) : '?'} Food:${bot.food ?? '?'} Fails:${failStreak}`);
  } else if (msg === '!help') {
...
function createBot() {
  ...
  log(`Connecting to ${host}:${port} [${serverIndex + 1}/${SERVERS.length}] (fail streak: ${failStreak})...`);
  ...
  bot.once('spawn', () => {
    failStreak   = 0;
    serverOnline = true;
    log(`✅ Spawned on ${host}:${port} — anti-AFK active.`);
    startAntiAfk();
    startStatusLogger();
  });
  ...
  bot.on('error', (err) => {
    ...
    if (code === 'ECONNREFUSED' || code === 'ETIMEDOUT' || code === 'ENOTFOUND') {
      serverOnline = false;
      log(`🔴 ${host}:${port} is offline (${code}) — will keep retrying...`);
    } else {
      serverOnline = true;
...
// ─── HTTP Status Server (for UptimeRobot) ────────────────────────────────────
...
const server = http.createServer((req, res) => {
  ...
  const body = JSON.stringify({
    status:    bot && bot.entity ? 'ONLINE' : (serverOnline ? 'RECONNECTING' : 'SERVER_OFFLINE'),
    server:    `${host}:${port}`,
    username:  USERNAME,
...
server.listen(HTTP_PORT, '0.0.0.0', () => log(`🌐 Status server running on port ${HTTP_PORT}`));
